...
Require-Match $migration '(?s)IF NOT EXISTS \(SELECT \* FROM sys\.objects WHERE object_id = OBJECT_ID\(N''\[dbo\]\.\[CLAIM_KS4_IMPORT_FILE\]''\).*?\r?\nEND\s*\r?\nGO\s*\r?\nALTER PROCEDURE \[dbo\]\.\[CLAIM_KS4_IMPORT_FILE\]' 'ACL migration must start ALTER PROCEDURE in a new batch.'
