/*
RollbackForMigrationId: 20260924_002_export_legacy_module_permissions
*/
-- AUTHORING ONLY. Separate G4 reversal approval, closed admission and completed
-- drain/reconciliation are prerequisites. Use the original exact public certificate
-- and signature packet, with #S11LegacyDeployment.Action='rollback'.
-- Remove application membership only through the separately approved reverse plan.
SET NOCOUNT ON;
SET XACT_ABORT ON;
DECLARE @Action varchar(16)='rollback';
...
EXEC @LockResult=sys.sp_getapplock @Resource=N'K98:S11:legacy-permissions',@LockMode='Exclusive',@LockOwner='Transaction',@LockTimeout=0;
IF @LockResult<0 THROW 51730,'Legacy permission reversal is busy.',1;
IF NOT EXISTS(SELECT 1 FROM sys.extended_properties WHERE class=0 AND name=N'S11LegacyPermissionManifest' AND CONVERT(nvarchar(64),value) COLLATE Latin1_General_100_BIN2=@ManifestHash)
 THROW 51730,'Exact delivery marker required; no blind rollback.',1;
...
IF EXISTS(SELECT 1 FROM sys.database_role_members WHERE role_principal_id=DATABASE_PRINCIPAL_ID(N'ExportLegacyEntryReader'))
 THROW 51730,'Entry role still has members; approved drain and unmapping required.',1;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.ARCHIVE_IMPORT_STAGING_FILE BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.CLAIM_KS4_IMPORT_FILE BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.CREATE_DASH2 BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.CREATE_DELTA_TABLES BY CERTIFICATE S11LegacyTargets;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.CREATE_THE_AVERAGES BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.GOVERNOR_NAMES_PROC BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.HASH_KS4_IMPORT_ARCHIVE_FILE BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.IMPORT_STAGING_PROC_CORE BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.Refresh_PlayerScanMeta BY CERTIFICATE S11LegacyImport;
DROP SIGNATURE FROM OBJECT::dbo.SP_Stats_for_Upload BY CERTIFICATE S11LegacyStats;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.SUMMARY_PROC BY CERTIFICATE S11LegacyImport;
DROP SIGNATURE FROM OBJECT::dbo.UPDATE_ALL2 BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Build_Prekvk_And_Honor_Rankings BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Build_Prekvk_And_Honor_Rankings BY CERTIFICATE S11LegacyTargets;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Create_Excel_For_Kvk_Indexes BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Create_Excel_For_Kvk_Indexes BY CERTIFICATE S11LegacyTargets;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_ExcelOutput_ByKVK BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_ExcelOutput_ByKVK BY CERTIFICATE S11LegacyTargets;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Prep_ExcelExportTable BY CERTIFICATE S11LegacyTargets;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Prep_ExcelOutputTable BY CERTIFICATE S11LegacyTargets;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Prep_TargetTable BY CERTIFICATE S11LegacyTargets;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Rebuild_ExcelForDashboard BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_RefreshInactiveGovernors BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Refresh_View_EXCEL_FOR_KVK_All BY CERTIFICATE S11LegacyImport;
DROP COUNTER SIGNATURE FROM OBJECT::dbo.sp_Refresh_View_EXCEL_FOR_KVK_All BY CERTIFICATE S11LegacyTargets;
DROP SIGNATURE FROM OBJECT::dbo.sp_TARGETS_MASTER BY CERTIFICATE S11LegacyTargets;
EXEC master.sys.sp_executesql N'REVOKE EXECUTE ON OBJECT::[dbo].[xp_fileexist] FROM [S11LegacyImportUser];';
EXEC master.sys.sp_executesql N'REVOKE EXECUTE ON OBJECT::[dbo].[xp_cmdshell] FROM [S11LegacyImportUser];';
EXEC master.sys.sp_executesql N'REVOKE VIEW SERVER PERFORMANCE STATE FROM [S11LegacyImportLogin];';
EXEC master.sys.sp_executesql N'REVOKE ADMINISTER BULK OPERATIONS FROM [S11LegacyImportLogin];';
REVOKE SELECT ON SCHEMA::[dbo] FROM [S11LegacyTargetsUser];
REVOKE INSERT ON SCHEMA::[dbo] FROM [S11LegacyTargetsUser];
REVOKE ALTER ON SCHEMA::[dbo] FROM [S11LegacyTargetsUser];
REVOKE VIEW DATABASE PERFORMANCE STATE FROM [S11LegacyTargetsUser];
REVOKE CREATE VIEW FROM [S11LegacyTargetsUser];
REVOKE CREATE TABLE FROM [S11LegacyTargetsUser];
REVOKE SELECT ON SCHEMA::[dbo] FROM [S11LegacyStatsUser];
REVOKE ALTER ON OBJECT::[dbo].[STATS_FOR_UPLOAD] FROM [S11LegacyStatsUser];
REVOKE SELECT ON SCHEMA::[dbo] FROM [S11LegacyImportUser];
REVOKE INSERT ON SCHEMA::[dbo] FROM [S11LegacyImportUser];
REVOKE ALTER ON SCHEMA::[dbo] FROM [S11LegacyImportUser];
REVOKE VIEW DATABASE PERFORMANCE STATE FROM [S11LegacyImportUser];
REVOKE CREATE VIEW FROM [S11LegacyImportUser];
REVOKE CREATE TABLE FROM [S11LegacyImportUser];
REVOKE CHECKPOINT FROM [S11LegacyImportUser];
REVOKE EXECUTE ON OBJECT::[dbo].[sp_Upsert_ProcConfig_From_Staging] FROM [ExportLegacyEntryReader];
REVOKE EXECUTE ON OBJECT::[dbo].[sp_TARGETS_MASTER] FROM [ExportLegacyEntryReader];
REVOKE EXECUTE ON OBJECT::[dbo].[UPDATE_ALL2] FROM [ExportLegacyEntryReader];
REVOKE EXECUTE ON OBJECT::[dbo].[SP_Stats_for_Upload] FROM [ExportLegacyEntryReader];
REVOKE EXECUTE ON OBJECT::[KVK].[sp_KVK_Recompute_Windows] FROM [ExportLegacyEntryReader];
REVOKE EXECUTE ON OBJECT::[KVK].[sp_KVK_Get_Exports] FROM [ExportLegacyEntryReader];
REVOKE EXECUTE ON OBJECT::[KVK].[sp_KVK_AllPlayers_Ingest] FROM [ExportLegacyEntryReader];

DROP ROLE ExportLegacyEntryReader;
DROP USER S11LegacyStatsUser;
DROP USER S11LegacyTargetsUser;
DROP USER S11LegacyImportUser;
EXEC master.sys.sp_executesql N'DROP USER S11LegacyImportUser; DROP LOGIN S11LegacyImportLogin;';
EXEC sys.sp_dropextendedproperty @name=N'S11LegacyPermissionManifest';
...
BEGIN CATCH
 ...
 THROW;
END CATCH;
-- Retain certificates, signature packet, all claims/journals and uncertain publications.
-- Migration-history reconciliation belongs to the exact approved reverse deployment.
