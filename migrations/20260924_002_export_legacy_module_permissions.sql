/*
MigrationId: 20260924_002_export_legacy_module_permissions
Purpose: Exact legacy root signatures and nested countersignatures; no procedure rewrite
Author: cwatts
CreatedUtc: 2026-09-24
RequiresBackup: Yes
RiskLevel: High
Rollback: Included
RollbackScript: migrations/rollback/20260924_002_export_legacy_module_permissions_rollback.sql
TransactionMode: Auto
DataChange: No
DataSafetyPlan: Included
EstimatedRowsAffected: 0 application rows
PreValidationQuery: Approved source/target/signature packet; backup and actual restore; admission drained
PostValidationQuery: validation/kvk_source/s11_export_legacy_module_permissions.sql
RelatedBotPR:
RelatedSQLPR:
*/
-- AUTHORING ONLY. This script is NOT installation approval.
-- G4 must provide public-only certificates in ROK_TRACKER (three) and master
-- (S11LegacyImport only), and the following session-local input tables:
-- #S11LegacyDeployment(ExpectedServer nvarchar(128), ManifestHash char(64),
--     ApprovalHash binary(32), Action varchar(16)); exactly one row, Action='install'.
-- #S11LegacySignatures(ModuleName nvarchar(257), CertificateName sysname,
--     CryptType char(4), Thumbprint varbinary(20), Signature varbinary(8000));
-- exactly the source-listed pairs, with independently approved exported blobs.
-- No private key, xp_cmdshell enablement, proxy credential, ACL, root call or
-- application role membership is created here. Unknown existing state refuses.
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF @LockResult<0 THROW 51730,'Legacy permission delivery is busy.',1;
DECLARE @ExistingManifest nvarchar(64)=(SELECT CONVERT(nvarchar(64),value) FROM sys.extended_properties WHERE class=0 AND name=N'S11LegacyPermissionManifest');
IF @ExistingManifest IS NOT NULL AND @ExistingManifest COLLATE Latin1_General_100_BIN2<>@ManifestHash
 THROW 51730,'A different permission delivery exists; preserve and reconcile.',1;
IF @ExistingManifest IS NULL
BEGIN

IF EXISTS(SELECT 1 FROM sys.database_principals WHERE name IN (N'S11LegacyImportUser',N'S11LegacyTargetsUser',N'S11LegacyStatsUser',N'ExportLegacyEntryReader'))
 OR EXISTS(SELECT 1 FROM master.sys.database_principals WHERE name=N'S11LegacyImportUser')
 OR EXISTS(SELECT 1 FROM sys.server_principals WHERE name=N'S11LegacyImportLogin')
 OR EXISTS(SELECT 1 FROM sys.crypt_properties p WHERE p.major_id IN (SELECT OBJECT_ID(ModuleName) FROM @Modules)
 OR p.thumbprint IN (SELECT Thumbprint FROM #S11LegacySignatures))
 THROW 51730,'Existing names/signatures are not adopted; reconcile the pre-change evidence.',1;
CREATE USER S11LegacyImportUser FOR CERTIFICATE S11LegacyImport;
CREATE USER S11LegacyTargetsUser FOR CERTIFICATE S11LegacyTargets;
CREATE USER S11LegacyStatsUser FOR CERTIFICATE S11LegacyStats;
CREATE ROLE ExportLegacyEntryReader AUTHORIZATION dbo;
EXEC master.sys.sp_executesql N'CREATE USER S11LegacyImportUser FOR CERTIFICATE S11LegacyImport;
CREATE LOGIN S11LegacyImportLogin FROM CERTIFICATE S11LegacyImport;';
...
GRANT CHECKPOINT TO [S11LegacyImportUser];
GRANT CREATE TABLE TO [S11LegacyImportUser];
GRANT CREATE VIEW TO [S11LegacyImportUser];
GRANT VIEW DATABASE PERFORMANCE STATE TO [S11LegacyImportUser];
GRANT ALTER ON SCHEMA::[dbo] TO [S11LegacyImportUser];
GRANT INSERT ON SCHEMA::[dbo] TO [S11LegacyImportUser];
GRANT SELECT ON SCHEMA::[dbo] TO [S11LegacyImportUser];
GRANT ALTER ON OBJECT::[dbo].[STATS_FOR_UPLOAD] TO [S11LegacyStatsUser];
GRANT SELECT ON SCHEMA::[dbo] TO [S11LegacyStatsUser];
GRANT CREATE TABLE TO [S11LegacyTargetsUser];
GRANT CREATE VIEW TO [S11LegacyTargetsUser];
GRANT VIEW DATABASE PERFORMANCE STATE TO [S11LegacyTargetsUser];
GRANT ALTER ON SCHEMA::[dbo] TO [S11LegacyTargetsUser];
GRANT INSERT ON SCHEMA::[dbo] TO [S11LegacyTargetsUser];
GRANT SELECT ON SCHEMA::[dbo] TO [S11LegacyTargetsUser];
EXEC master.sys.sp_executesql N'GRANT ADMINISTER BULK OPERATIONS TO [S11LegacyImportLogin];';
EXEC master.sys.sp_executesql N'GRANT VIEW SERVER PERFORMANCE STATE TO [S11LegacyImportLogin];';
EXEC master.sys.sp_executesql N'GRANT EXECUTE ON OBJECT::[dbo].[xp_cmdshell] TO [S11LegacyImportUser];';
EXEC master.sys.sp_executesql N'GRANT EXECUTE ON OBJECT::[dbo].[xp_fileexist] TO [S11LegacyImportUser];';

DECLARE @CertificateName sysname,@CryptType char(4),@Signature varbinary(8000),@SignSql nvarchar(max);
DECLARE signatures CURSOR LOCAL FAST_FORWARD FOR SELECT ModuleName,CertificateName,CryptType,Signature FROM #S11LegacySignatures ORDER BY ModuleName,CertificateName;
OPEN signatures;
FETCH NEXT FROM signatures INTO @ModuleName,@CertificateName,@CryptType,@Signature;
WHILE @@FETCH_STATUS=0
BEGIN
 SET @SignSql=N'ADD '+CASE WHEN @CryptType='CPVC' THEN N'COUNTER ' ELSE N'' END+N'SIGNATURE TO OBJECT::'
 +QUOTENAME(PARSENAME(@ModuleName,2))+N'.'+QUOTENAME(PARSENAME(@ModuleName,1))+N' BY CERTIFICATE '+QUOTENAME(@CertificateName)
 +N' WITH SIGNATURE = '+CONVERT(nvarchar(max),@Signature,1)+N';';
 EXEC sys.sp_executesql @SignSql;
 FETCH NEXT FROM signatures INTO @ModuleName,@CertificateName,@CryptType,@Signature;
END;
CLOSE signatures;
DEALLOCATE signatures;
EXEC sys.sp_addextendedproperty @name=N'S11LegacyPermissionManifest',@value=@ManifestHash;
END;
