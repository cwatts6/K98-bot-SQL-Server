...
BEGIN
    ...
    (
        SELECT TRY_CONVERT(bigint, history.[Gov_ID]) AS Gov_ID,
               ...
               TRY_CONVERT(float, history.[Acclaim]) AS Acclaim,
               TRY_CONVERT(float, history.[T4&T5_Kills]) AS Kills,
               TRY_CONVERT(float, history.[KillPointsDelta]) AS KillPoints,
               TRY_CONVERT(float, history.[Deads_Delta]) AS Deads,
               TRY_CONVERT(float, history.[HealedTroopsDelta]) AS Healed,
               TRY_CONVERT(float, history.[DKP_SCORE]) AS DKP,
               TRY_CONVERT(float, history.[Max_PreKvk_Points]) AS PreKvk,
               TRY_CONVERT(float, history.[Max_HonorPoints]) AS Honor,
               combat.TankingScore,
               combat.IsEngaged
        FROM dbo.v_EXCEL_FOR_KVK_All AS history
