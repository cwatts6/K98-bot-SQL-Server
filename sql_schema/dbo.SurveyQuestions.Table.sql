...
CREATE TABLE [dbo].[SurveyQuestions](
	...
	[RatingMinValue] [tinyint] NOT NULL,
	[RatingMaxValue] [tinyint] NOT NULL,
	[RatingLowLabel] [nvarchar](40) COLLATE Latin1_General_CI_AS NULL,
	[RatingHighLabel] [nvarchar](40) COLLATE Latin1_General_CI_AS NULL,
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_SurveyQuestions_RatingMinValue]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[SurveyQuestions] ADD  CONSTRAINT [DF_SurveyQuestions_RatingMinValue]  DEFAULT ((1)) FOR [RatingMinValue]
END

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DF_SurveyQuestions_RatingMaxValue]') AND type = 'D')
BEGIN
ALTER TABLE [dbo].[SurveyQuestions] ADD  CONSTRAINT [DF_SurveyQuestions_RatingMaxValue]  DEFAULT ((5)) FOR [RatingMaxValue]
END
...
IF NOT EXISTS (SELECT * FROM sys.check_constraints WHERE object_id = OBJECT_ID(N'[dbo].[CK_SurveyQuestions_RatingScale]') AND parent_object_id = OBJECT_ID(N'[dbo].[SurveyQuestions]'))
ALTER TABLE [dbo].[SurveyQuestions]  WITH CHECK ADD  CONSTRAINT [CK_SurveyQuestions_RatingScale] CHECK  (([QuestionType]='Rating' AND ([RatingMinValue]>=(1) AND [RatingMinValue]<=(9)) AND ([RatingMaxValue]>=(2) AND [RatingMaxValue]<=(10)) AND [RatingMaxValue]>[RatingMinValue] OR [QuestionType]<>'Rating' AND [RatingMinValue]=(1) AND [RatingMaxValue]=(5) AND [RatingLowLabel] IS NULL AND [RatingHighLabel] IS NULL))
IF  EXISTS (SELECT * FROM sys.check_constraints WHERE object_id = OBJECT_ID(N'[dbo].[CK_SurveyQuestions_RatingScale]') AND parent_object_id = OBJECT_ID(N'[dbo].[SurveyQuestions]'))
ALTER TABLE [dbo].[SurveyQuestions] CHECK CONSTRAINT [CK_SurveyQuestions_RatingScale]
