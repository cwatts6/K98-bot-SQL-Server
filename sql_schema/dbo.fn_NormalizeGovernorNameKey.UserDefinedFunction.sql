...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[fn_NormalizeGovernorNameKey]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
execute dbo.sp_executesql @statement = N'CREATE FUNCTION [dbo].[fn_NormalizeGovernorNameKey](@GovernorName [nvarchar](255))
RETURNS [nvarchar](100) WITH SCHEMABINDING, EXECUTE AS CALLER
AS 
BEGIN
