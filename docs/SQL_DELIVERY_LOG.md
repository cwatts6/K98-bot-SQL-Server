...
### S10C final offline authoring checkpoint — 2026-09-14

The exact approved eight-path SQL union was checked against fresh status, with no extra or
missing paths and an empty index. HEAD/origin main remains the accepted
`3776dfa6b0892a8800d236fdf111c4d2f93c3813`; no fetch, pull, commit or publication occurred.
`deploy/Test-LegacyExportPreparationContracts.ps1` passed full CREATE-body comparisons
between both new snapshots and the migration, plus required ownership/drift metadata checks.
The migration seals installed column, CHECK/default/FK and index metadata and rejects an
unsealed or drifted rerun. The authored fixture includes negative scope, missing capture,
zero-version, missing-owner and membership cases and retains the resource after rejected changes.
These SQL cases were not executed. The Bot's distinct S10C opt-in case is also skipped offline.
SQL Changes/Deep-off review remains independently pending at this checkpoint. No database
execution, current-provider evidence, actual restore, deployment or S6/S8 gate closure is claimed.
