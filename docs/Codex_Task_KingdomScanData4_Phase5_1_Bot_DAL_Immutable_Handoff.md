...
## 18. Phase 5.2 Checkpoint A Closure Addendum — 2026-08-17

This addendum does not rewrite the historical Phase 5.1 scope or its accepted evidence receipt.
Checkpoint A found that the ACL configuration script normalized only the Archive root, so a
pre-existing protected child ACL could survive. The approved narrow remediation now normalizes and
verifies all Archive descendants while preserving their path inventory. Its first commit exposed a
second Low hard-link redirection condition, which the final commit rejects before privileged
`icacls /reset` or `/setowner` execution.

- Replacement mirror PR #232 range:
  `46e5a9cd58a4f475557904226656b2b8cc39dbb2..f95ead9d348bdf45726fb9ce1e73f6ed2a20483a`;
  29 changed files, clean/mergeable, no unresolved review threads, and no GitHub jobs.
- Validation: focused immutable-handoff tests `9 passed`; full pytest `2823 passed, 2 skipped`;
  pre-commit, PowerShell parsing, K98 architecture/deferred/security-routing/command validators,
  disposable protected-child normalization, reparse-point refusal, and hard-link refusal passed.
- Exact Changes scan `02d8e353-4eec-40e9-bafa-4fd4c53ac860` reported
  `csf_38e8134cbe97537f0652c431`; closure scan
  `0ecf75bf-359c-4503-a6c8-3fcbed84c98e` reported zero findings. Together with original exact-range
  scan `c6761a00-3670-48f5-965f-43fe3228e675`, coverage is contiguous through the replacement
  head. Deep was off for every scan.
- Replacement production PR #539 head `53eaeb66b99538778ad7cd95a974dcd0bc8ccd55` was generated by
  the patch-based promotion script from production `main`
  `caabd2c7dc77aec67f2748a1b9b66fdf53a4aa02`; all 29 file objects match the mirror and both
  quality and scan checks pass.
- The successful Phase 5.1 receipt remains accepted historical proof for its recorded SQL and bot
  heads. Phase 5.2 must rehearse the replacement frozen heads; no merge, deployment, restart,
  production SQL change, or restore is authorized by this addendum.
