...
foreach ($source in @($canonical, $migration)) {
    ...
    Assert-Contains $source 'header\.SnapshotId,\s*header\.WeekStartUtc' 'Alliance Activity observations must carry their weekly-reset boundary.'
    Assert-Contains $source 'LAG\(ActivityWeekStartDate\)' 'Alliance Activity comparisons must retain the prior weekly-reset boundary.'
    Assert-Contains $source 'TechValue\s*>\s*0[\s\S]+PreviousTechValue\s+IS\s+NOT\s+NULL[\s\S]+ActivityWeekStartDate\s*>\s*comparison\.PreviousActivityWeekStartDate' 'Positive Tech Donations in a later observed week must qualify from the weekly zero baseline without treating missing evidence as zero.'
    Assert-Contains $source 'BuildingValue\s*>\s*0[\s\S]+PreviousBuildingValue\s+IS\s+NOT\s+NULL[\s\S]+ActivityWeekStartDate\s*>\s*comparison\.PreviousActivityWeekStartDate' 'Positive Building Minutes in a later observed week must qualify from the weekly zero baseline without treating missing evidence as zero.'
