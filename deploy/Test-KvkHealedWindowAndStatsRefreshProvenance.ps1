...
Assert-Contains $stats '@LatestKVK int' 'Latest KVK must retain the ProcConfig KVKVersion integer type.'
Assert-Contains $stats '@pLatestKVK = @LatestKVK' 'Dynamic SQL must bind the latest KVK through a local variable.'
Assert-NotContains $stats '@pLatestKVK\s*=\s*CONVERT\s*\(' 'EXEC parameter bindings must not contain an inline CONVERT expression.'
